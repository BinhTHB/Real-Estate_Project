...
def reading_delta_table(context, s3_path_property) -> Tuple[pd.DataFrame, DeltaTable]:
    ...
    try:
        ...
        # SỬA LẠI: Nếu không tìm thấy bảng, chỉ trả về None.
        # Hàm merge_property_delta (nơi gọi) sẽ xử lý việc tạo bảng mới.
        context.log.warning(f"Table not found at {s3_path_property}. Will be created by caller.")
        df = None
        dt = None
